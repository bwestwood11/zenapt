...
export function OrganizationPromoCodes() {
  const queryClient = useQueryClient();

  const { data: promoCodes, isLoading } = useQuery(
    trpc.location.listOrganizationPromoCodes.queryOptions(),
  );

  const { mutate: createPromoCode, isPending: isCreating } = useMutation(
    trpc.location.createOrganizationPromoCode.mutationOptions({
      onSuccess: () => {
        toast.success("Organization promo code created");
        form.reset({ code: "", description: "", discount: 10, maxUsage: undefined });
        queryClient.invalidateQueries({
          queryKey: trpc.location.listOrganizationPromoCodes.queryKey(),
        });
      },
      onError: (error) => {
  ...
  const { mutate: updatePromoCode, isPending: isUpdating } = useMutation(
    trpc.location.updateOrganizationPromoCode.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: trpc.location.listOrganizationPromoCodes.queryKey(),
        });
  ...
  const { mutate: deletePromoCode, isPending: isDeleting } = useMutation(
    trpc.location.deleteOrganizationPromoCode.mutationOptions({
      onSuccess: () => {
  ...
  let promoCodeListContent: React.ReactNode = (
    <p className="text-sm text-muted-foreground">
      No organization-level promo codes yet.
    </p>
  );
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle>Organization Promo Codes</CardTitle>
        </div>
        <CardDescription>
          Create promo codes available across all locations in your organization.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} value={field.value ?? ""} placeholder="Organization-wide promotion" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <div className="space-y-3">
          <Label>Existing organization promo codes</Label>
          {promoCodeListContent}
        </div>
      </CardContent>
    </Card>
