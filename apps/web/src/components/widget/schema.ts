...
export const CartSchema = z.object({
  ...
  employeeServiceId: z
    .string({ error: "You must select an employee to perform the service." })
    .min(1, { error: "You must select an employee to perform the service." }),
  employeeId: z.string({ error: "Employee ID is required." }).min(1).optional(),
  addons: z
    .array(
      z.object({
