...
const getPermission = protectedProcedure
  .query(async ({ ctx, input }) => {
    const userAccessContext = await getUserAccessContext(ctx.session.user.id);
    const orgId = ctx.session.user.organizationId

    if(!orgId) {
      return {organizationRoles: null, locationsRoles:null, organizationPermissions: null, locationPermissions: null}
    }
    ...
    return { organizationRoles, locationRoles, locationPermissions, organizationPermissions };
  });
